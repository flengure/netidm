...
This section will guide you through a quick setup of Netidm for evaluation. It's recommended that for a production
...
```bash
docker pull docker.io/netidm/server:latest
```
...
docker create --name netidmd \
  -p '443:8443' \
  -p '636:3636' \
  -v netidmd:/data \
  docker.io/netidm/server:latest
```

## Copy the configuration to the container

```bash
docker cp server.toml netidmd:/data/server.toml
```
...
docker run --rm -i -t -v netidmd:/data \
  docker.io/netidm/server:latest \
  netidmd cert-generate
```

## Start Netidmd Container

```bash
docker start netidmd
```

## Recover the Admin Role Passwords

The `admin` account is used to configure Netidm itself.

```bash
docker exec -i -t netidmd \
...
Netidm requires locally installed client tools on the system used for administration via the command line. Follow the
...
After installing the `netidm` tool, you will need to create a configuration file to continue.
...
```shell
netidm person create <your username> <Your Displayname>
```

## Set up your account credentials

```shell
netidm person credential create-reset-token <your username>
```
...
- [Replication](repl/), if one Netidm instance isn't enough
