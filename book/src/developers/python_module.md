...
- every class and function is fully python typed (test by running `make test/pynetidm/mypy`)
...
2. Build the base environment. From within the `pynetidm` directory, run: `uv sync` This'll set up a virtual environment
   and install all the required packages (and development-related ones)
3. Start editing!

Most IDEs will be happier if you open the netidm_rlm_python or pynetidm directories as the base you are working from,
rather than the netidm repository root, so they can auto-load integrations etc.
...
```shell
make docs/pynetidm/build
```

You can also run a local live server by running:

```shell
make docs/pynetidm/serve
```
