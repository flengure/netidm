...
2. Build the base environment. From within the netidm_rlm_python directory, run: `uv sync`
3. Install the `netidm` python library: `uv run python -m pip install ../pynetidm`
4. Start editing!

Most IDEs will be happier if you open the `netidm_rlm_python` or `pynetidm` directories as the base you are working
from, rather than the `netidm` repository root, so they can auto-load integrations etc.

## Running a test RADIUS container

From the root directory of the Netidm repository:

1. Build the container - this'll give you a container image called `netidm/radius` with the tag `devel`:
...
```bash
cd netidm_rlm_python && ./run_radius_container.sh
```

You can pass the following environment variables to `run_radius_container.sh` to set other options:

- IMAGE: an alternative image such as `ghcr.io/netidm/radius:devel`
- CONFIG_FILE: mount your own config file

For example:

```bash
IMAGE=ghcr.io/netidm/radius:devel \
    CONFIG_FILE=~/.config/netidm \
