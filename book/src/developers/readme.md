...
All configuration items, must be constrained to fit within our principles so that every Netidm deployment, will aim to
provide a positive experience to all people.

## Setup the Server

It's important before you start trying to write code and contribute that you understand what Netidm does and its goals.
...
Each operating system has different steps required to configure and build Netidm.
...
> NOTE: clang and lld are required to build Netidm due to performance issues with GCC/ld
...
Other supporting projects can be found on the [project github](https://github.com/netidm)

If you are forking, then fork in GitHub and then add your remote.

```bash
git remote add myfork git@github.com:<YOUR USERNAME>/netidm.git
```
...
> [!IMPORTANT]
>
> Netidm is unable to accept code that is generated by an AI for legal reasons. copilot and other tools that generate
> code in this way can not be used in Netidm.
...
A list of links to the library documentation is at [netidm.com/documentation](https://netidm.com/documentation/).
...
Build profiles allow us to change the operation of Netidm during it's compilation for development or release on various
platforms. By default the "developer" profile is used that assumes the correct relative paths within the monorepo.

Setting different developer profiles while building is done by setting the environment variable `NETIDM_BUILD_PROFILE`
to one of the bare filename of the TOML files in `/profiles`.

For example, this will set the CPU flags to "none" and the location for the Web UI files to `/usr/share/netidm/ui/pkg`:

```bash
NETIDM_BUILD_PROFILE=release_linux cargo build --release --bin netidmd
```
...
We recommend using [Let's Encrypt](https://letsencrypt.org), but if this is not possible netidmd will create self-signed
certificates in `/tmp/netidm`.

You can now build and run the server with the commands below. It will use a database in `/tmp/netidm/netidm.db`.
...
```bash
cargo run --bin netidm -- --help
cargo run --bin netidm -- login -H https://localhost:8443 -D anonymous -C /tmp/netidm/ca.pem
cargo run --bin netidm -- self whoami -H https://localhost:8443 -D anonymous -C /tmp/netidm/ca.pem

cargo run --bin netidm -- login -H https://localhost:8443 -D admin -C /tmp/netidm/ca.pem
cargo run --bin netidm -- self whoami -H https://localhost:8443 -D admin -C /tmp/netidm/ca.pem
```

You may find it easier to modify `~/.config/netidm` per the [book client tools section](../client_tools.md) for extended
...
netidm raw create -H https://localhost:8443 -C ../insecure/ca.pem -D admin example.create.account.json
netidm raw create  -H https://localhost:8443 -C ../insecure/ca.pem -D idm_admin example.create.group.json

# Apply a json stateful modification to all entries matching a filter
netidm raw modify -H https://localhost:8443 -C ../insecure/ca.pem -D admin '{"or": [ {"eq": ["name", "idm_person_account_create_priv"]}, {"eq": ["name", "idm_service_account_create_priv"]}, {"eq": ["name", "idm_account_write_priv"]}, {"eq": ["name", "idm_group_write_priv"]}, {"eq": ["name", "idm_people_write_priv"]}, {"eq": ["name", "idm_group_create_priv"]} ]}' example.modify.idm_admin.json
netidm raw modify -H https://localhost:8443 -C ../insecure/ca.pem -D idm_admin '{"eq": ["name", "idm_admins"]}' example.modify.idm_admin.json

# Search and show the database representations
netidm raw search -H https://localhost:8443 -C ../insecure/ca.pem -D admin '{"eq": ["name", "idm_admin"]}'

# Delete all entries matching a filter
netidm raw delete -H https://localhost:8443 -C ../insecure/ca.pem -D idm_admin '{"eq": ["name", "test_account_delete_me"]}'
```

### Build a Netidm Container
...
| ---------------------- | --------------------------------------------------------- | ------------------------- |
| `IMAGE_BASE`           | Base location of the container image.                     | `netidm`                  |
...
Build a `netidm` container using `podman`:

```bash
CONTAINER_TOOL=podman make build/netidmd
```

Build a `netidm` container and use a redis build cache:

```bash
CONTAINER_BUILD_ARGS='--build-arg "SCCACHE_REDIS=redis://redis.dev.blackhats.net.au:6379"' make build/netidmd
```
...
Whenever code is merged with the `master` branch of Netidm, containers are automatically built for `netidmd` and
`radius`. Sometimes they fail to build, but we'll try to keep them available.

To find information on the packages,
[visit the Netidm packages page](https://github.com/orgs/netidm/packages?repo_name=netidm).
...
```bash
docker pull ghcr.io/netidm/radius:devel
docker run --rm -it \
    -v $(pwd)/netidm:/data/netidm \
    ghcr.io/netidm/radius:devel
```

This assumes you have a `netidm` client configuration file in the current working directory.
