...
netidm service-account create demo_service "Demonstration Service" demo_group --name idm_admin
netidm service-account get demo_service --name idm_admin
```
...
```bash
netidm service-account api-token status --name ENTRY_MANAGER ACCOUNT_ID
netidm service-account api-token status --name demo_user demo_service
```
...
```bash
netidm service-account api-token generate --name ENTRY_MANAGER ACCOUNT_ID LABEL [EXPIRY]
netidm service-account api-token generate --name demo_user demo_service "Test Token"
netidm service-account api-token generate --name demo_user demo_service "Test Token" 2020-09-25T11:22:02+10:00
```

If you wish to issue a token that is able to make changes on behalf of the service account, you must add the `-w` flag
during the generate command. It is recommended you only add `-w` when the API token is performing writes to Netidm.

```bash
netidm service-account api-token generate --name ENTRY_MANAGER ACCOUNT_ID LABEL [EXPIRY] --readwrite
netidm service-account api-token generate --name demo_user demo_service "Test Token" --readwrite
netidm service-account api-token generate --name demo_user demo_service "Test Token" 2020-09-25T11:22:02+10:00 --readwrite
```
...
netidm service-account api-token status --name ENTRY_MANAGER ACCOUNT_ID
netidm service-account api-token status --name demo_user demo_service
netidm service-account api-token destroy --name ENTRY_MANAGER ACCOUNT_ID TOKEN_ID
netidm service-account api-token destroy --name demo_user demo_service 4de2a4e9-e06a-4c5e-8a1b-33f4e7dd5dc7
```

### API Tokens with Netidm HTTPS/REST API
...
- `subject_token`: the service-account API token (see `netidm service-account api-token generate`)
...
API Tokens issued by Netidm are a JWS object that contains metadata about the service account that is being granted
...
fail validation and Netidm will refuse it.
...
```bash
netidm service-account api-token generate --name ENTRY_MANAGER ACCOUNT_ID LABEL [EXPIRY] --compact
netidm service-account api-token generate --name demo_user demo_service "Test Token" --compact
```
