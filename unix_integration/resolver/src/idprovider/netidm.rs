...
use netidm_lib_crypto::DbPasswordV1;
use netidm_lib_crypto::Password;
...
use netidm_unix_common::unix_config::{GroupMap, NetidmConfig};
use netidm_unix_common::unix_proto::PamAuthRequest;
use std::collections::BTreeSet;
use std::time::{Duration, SystemTime};
use tokio::sync::{broadcast, Mutex};

const NETIDM_HMAC_KEY: &str = "netidm-hmac-key-v2";
const NETIDM_PWV1_KEY: &str = "netidm-pw-v1";
...
pub struct NetidmProvider {
    inner: Mutex<NetidmProviderInternal>,
...
impl NetidmProvider {
    pub async fn new<'a, 'b>(
        ...
        let loadable_hmac_key: Option<LoadableHmacS256Key> = keystore
            .get_tagged_hsm_key(NETIDM_HMAC_KEY)
            .map_err(|ks_err| {
                error!(?ks_err);
                IdpError::KeyStore
            })?;
        ...
        let loadable_hmac_key = if let Some(loadable_hmac_key) = loadable_hmac_key {
            ...
            keystore
                .insert_tagged_hsm_key(NETIDM_HMAC_KEY, &loadable_hmac_key)
                .map_err(|ks_err| {
                    error!(?ks_err);
                    IdpError::KeyStore
                })?;
        ...
        Ok(NetidmProvider {
            inner: Mutex::new(NetidmProviderInternal {
...
impl From<UnixUserToken> for UserToken {
    fn from(value: UnixUserToken) -> UserToken {
        ...
        UserToken {
            provider: ProviderOrigin::Netidm,
            name,
            spn,
            uuid,
            gidnumber,
...
impl UserToken {
    pub fn netidm_update_cached_password(
        &mut self,
        crypto_policy: &CryptoPolicy,
        cred: &str,
        tpm: &mut BoxedDynTpm,
        ...
        let pw = match Password::new_argon2id_hsm(crypto_policy, cred, tpm_ctx, hmac_key) {
            ...
            Err(reason) => {
                // Clear cached pw.
                self.extra_keys.remove(NETIDM_PWV1_KEY);
                warn!(
                    ?reason,
        ...
        self.extra_keys.insert(NETIDM_PWV1_KEY.into(), pw_value);
        debug!(spn = %self.spn, "Updated cached pw");
    }

    pub fn netidm_has_offline_credentials(&self) -> bool {
        self.extra_keys.contains_key(NETIDM_PWV1_KEY)
    }

    pub fn netidm_check_cached_password(
        &self,
        cred: &str,
        tpm: &mut BoxedDynTpm,
        hmac_key: &HmacS256Key,
    ) -> bool {
        let pw_value = match self.extra_keys.get(NETIDM_PWV1_KEY) {
...
#[async_trait]
impl IdProvider for NetidmProvider {
    fn origin(&self) -> ProviderOrigin {
        ProviderOrigin::Netidm
    }
            ...
            (AuthCredHandler::Password, PamAuthRequest::Password { cred }) => {
                ...
                match auth_result {
                    Ok(Some(n_tok)) => {
                        ...
                        new_token.netidm_update_cached_password(
                            &inner.crypto_policy,
                            cred.as_str(),
                            tpm,
                            &inner.hmac_key,
    ...
    async fn unix_user_can_offline_auth(&self, token: &UserToken) -> bool {
        token.netidm_has_offline_credentials()
    }
    ...
    async fn unix_user_offline_auth_init(
        ...
        if token.netidm_has_offline_credentials() {
            Ok((AuthRequest::Password, AuthCredHandler::Password))
        } else {
    ...
    async fn unix_user_offline_auth_step(
        ...
        match (cred_handler, pam_next_req) {
            (AuthCredHandler::Password, PamAuthRequest::Password { cred }) => {
                ...
                if session_token.netidm_check_cached_password(cred.as_str(), tpm, &inner.hmac_key) {
