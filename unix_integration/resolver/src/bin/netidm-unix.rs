...
macro_rules! setup_client {
    () => {{
        ...
        debug!(
            "Using netidm_unixd socket path: {:?}",
            cfg.sock_path.as_str()
        );

        // see if the netidm_unixd socket exists and quit if not
...
async fn main() -> ExitCode {
    let opt = NetidmUnixParser::parse();

    let debug = match opt.commands {
        NetidmUnixOpt::AuthTest {
            debug,
            account_id: _,
        } => debug,
        NetidmUnixOpt::CacheClear { debug, really: _ } => debug,
        NetidmUnixOpt::CacheInvalidate { debug } => debug,
        NetidmUnixOpt::Status { debug } => debug,
        NetidmUnixOpt::Version { debug } => debug,
    };

    if debug {
        ::std::env::set_var("RUST_LOG", "netidm=debug,netidm_client=debug");
    }
    ...
    match opt.commands {
                ...
                match daemon_client.call(req, None).await {
                    Ok(r) => match r {
                        ...
                        ClientResponse::Error(err) => {
                            error!("Error from netidm-unixd: {}", err);
                            break;
                        }
                        ClientResponse::PamAuthenticateStepResponse { .. }
        ...
        NetidmUnixOpt::CacheClear { debug: _, really } => {
        ...
        NetidmUnixOpt::CacheInvalidate { debug: _ } => {
        ...
        NetidmUnixOpt::Version { debug: _ } => {
            println!("netidm-unix {}", env!("NETIDM_PKG_VERSION"));
            ExitCode::SUCCESS
        }
    }
}
