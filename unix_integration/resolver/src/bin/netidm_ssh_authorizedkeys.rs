...
use netidm_unix_common::unix_proto::{ClientRequest, ClientResponse};
...
async fn main() -> ExitCode {
    ...
    if opt.version {
        println!("ssh_authorizedkeys {}", env!("NETIDM_PKG_VERSION"));
    ...
    match daemon_client.call(req, None).await {
        ...
        Ok(r) => {
            error!("Error calling netidm_unixd: unexpected response -> {:?}", r);
            ExitCode::FAILURE
        }
        Err(e) => {
            error!("Error calling netidm_unixd -> {:?}", e);
            ExitCode::FAILURE
        }
    }
}
