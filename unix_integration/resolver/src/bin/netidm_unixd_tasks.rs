...
use netidm_unix_common::unix_config::{HomeStrategy, UnixdConfig};
use netidm_unix_common::unix_passwd::{parse_etc_group, parse_etc_passwd, parse_etc_shadow, EtcDb};
...
use netidm_unix_common::selinux_util;

static NETIDM_UNIX_RETRY_SECS: u64 = 5;
...
fn chown(path: &Path, gid: u32) -> Result<(), String> {
    ...
    // Change the owner to the gid - remember, netidm ONLY has gid's, the uid is implied.
...
async fn handle_tasks(
    ...
    loop {
        ...
        if let Err(e) = reqs.send(msg).await {
            error!(?e, "Error sending response to netidm_unixd");
            return;
        }
    }
}
...
async fn main() -> ExitCode {
    ...
    for arg in std::env::args() {
        if arg.contains("--version") {
            println!("netidm_unixd_tasks {}", env!("CARGO_PKG_VERSION"));
            return ExitCode::SUCCESS;
        } else if arg.contains("--help") {
            println!("netidm_unixd_tasks {}", env!("CARGO_PKG_VERSION"));
            println!("Usage: netidm_unixd_tasks");
            println!("  --version");
            println!("  --help");
            return ExitCode::SUCCESS;
        }
    ...
    tracing_forest::worker_task()
        ...
        .on(async {
            ...
            let server = tokio::spawn(async move {
                loop {
                    info!("Attempting to connect to netidm_unixd ...");
                    ...
                    tokio::select! {
                        ...
                        connect_res = UnixStream::connect(&task_sock_path) => {
                            match connect_res {
                                Ok(stream) => {
                                    info!("Found netidm_unixd, waiting for tasks ...");
                                ...
                                Err(e) => {
                                    debug!("\\---> {:?}", e);
                                    error!("Unable to find netidm_unixd, sleeping for {} seconds ...", NETIDM_UNIX_RETRY_SECS);
                                    // Back off.
                                    time::sleep(Duration::from_secs(NETIDM_UNIX_RETRY_SECS)).await;
                                }
                            }
                        }
                    } // select
