...
use netidm_unix_common::json_codec::JsonCodec;
use netidm_unix_common::unix_config::{HsmType, UnixdConfig};
use netidm_unix_common::unix_passwd::EtcDb;
use netidm_unix_common::unix_proto::{
    ClientRequest, ClientResponse, TaskRequest, TaskRequestFrame, TaskResponse,
};
use netidm_unix_resolver::idprovider::interface::IdProvider;
use netidm_unix_resolver::idprovider::netidm::NetidmProvider;
use netidm_unix_resolver::idprovider::system::SystemProvider;
use netidm_unix_resolver::resolver::{AuthSession, Resolver};
use netidm_unix_resolver::{
    check_nsswitch_has_netidm,
    db::{Cache, Db},
};
        ...
        tokio::select! {
            ...
            response = framed_stream.next() => {
                ...
                match response {
                    ...
                    other => {
                        let errmsg = format!("Received unexpected message from netidm-unixd-tasks: {:?}", other);
                        error!("{}", errmsg);
...
async fn main_inner(clap_args: clap::ArgMatches) -> ExitCode {
    ...
    debug!("Profile -> {}", env!("NETIDM_PROFILE_NAME"));
    debug!("CPU Flags -> {}", env!("NETIDM_CPU_FLAGS"));
    ...
    if !unixd_path.exists() {
        ...
        if !netidm_lib_file_permissions::readonly(&unixd_meta) {
            warn!("permissions on {} may not be secure. Should be readonly to running uid. This could be a security risk ...",
                        unixd_path_str);
        }
    ...
    let client_builder = if let Some(kconfig) = &cfg.netidm_config {
        if kconfig.pam_allowed_login_groups.is_empty() {
            error!("Netidm is enabled but no pam_allowed_login_groups are set - NETIDM USERS CANNOT AUTHENTICATE !!!");
        }

        // setup
        let cb = match NetidmClientBuilder::new().read_options_from_optional_config(&cfg_path) {
            Ok(v) => v,
            Err(_) => {
    ...
    check_nsswitch_has_netidm(None);

    if clap_args.get_flag("configtest") {
        eprintln!("###################################");
        eprintln!("Dumping configs:\n###################################");
        eprintln!("netidm_unixd config (from {:#?})", &unixd_path);
        eprintln!("{cfg}");
        eprintln!("###################################");
        if let Some((cb, _)) = client_builder.as_ref() {
            eprintln!("netidm client config (from {:#?})", &cfg_path);
            eprintln!("{cb}");
        } else {
            eprintln!("netidm client: disabled");
        }
    ...
    // Setup Netidm provider if the configuration requests it.
    if let Some((cb, kconfig)) = client_builder {
        ...
        let Ok(idprovider) = NetidmProvider::new(
            rsclient,
            kconfig,
            SystemTime::now(),
            &mut (&mut db_txn).into(),
        ...
        .await
            ...
            error!("Failed to configure Netidm Provider");
            return ExitCode::FAILURE;
        };

        // Now stacked for the resolver.
        clients.push(Arc::new(idprovider));
        info!("Started netidm provider");
    }
