...
# - set up a test user
# - set up a test group
# - set up a test oauth2 rp (https://netidm.com)
...
while true; do
    if [ "$(pgrep netidmd | wc -l)" -eq 1 ]; then
        break
    fi
    echo "Start the netidmd server first please!"

    while true; do
        echo "Waiting for you to start the server... testing ${NETIDM_URL}"
        curl --cacert "${NETIDM_CA_PATH}" -fs "${NETIDM_URL}" >/dev/null && break
        sleep 2
    done
done
...
export NETIDM_URL
export NETIDM_CA_PATH
export NETIDM_CONFIG_FILE
...
NETIDM="cargo run ${BUILD_MODE} --manifest-path ../../Cargo.toml --bin netidm -- "
NETIDMD="cargo run ${BUILD_MODE} -p daemon --bin netidmd -- "

if [ "${REMOVE_TEST_DB}" -eq 1 ]; then
    echo "Removing the existing DB!"
    rm /tmp/netidm/netidm.db || true
fi
...
IDM_ADMIN_PASS_RAW="$(${NETIDMD} scripting recover-account idm_admin 2>&1)"
...
${NETIDM} login -D "${IDM_ADMIN_USER}" --password "${IDM_ADMIN_PASS}"

# create group test_users
${NETIDM} group create "${TEST_GROUP}" -D "${IDM_ADMIN_USER}"

# create testuser (person)
${NETIDM} person create "${TEST_USER_NAME}" "${TEST_USER_DISPLAY}" -D "${IDM_ADMIN_USER}"

echo "Adding ${TEST_USER_NAME} to ${TEST_GROUP}"
${NETIDM} group add-members "${TEST_GROUP}" "${TEST_USER_NAME}" -D "${IDM_ADMIN_USER}"

echo "Enable experimental UI for admin idm_admin ${TEST_USER_NAME}"
${NETIDM} group add-members idm_ui_enable_experimental_features "${IDM_ADMIN_USER}" "${TEST_USER_NAME}" -D "${IDM_ADMIN_USER}"

# create oauth2 rp for netidm.com
echo "Creating the netidm.com OAuth2 RP"
${NETIDM} system oauth2 create "netidm_com" "Netidm.com" "https://netidm.com" -D "${IDM_ADMIN_USER}"
echo "Creating the netidm.com OAuth2 RP Scope Map"
${NETIDM} system oauth2 update-scope-map "netidm_com" "${TEST_GROUP}" openid -D "${IDM_ADMIN_USER}"
echo "Creating the netidm.com OAuth2 RP Supplemental Scope Map"
${NETIDM} system oauth2 update-sup-scope-map "netidm_com" "${TEST_GROUP}" admin -D "${IDM_ADMIN_USER}"


# create oauth2 rp for localhost:10443 - for oauth2 proxy testing
echo "Creating the ${OAUTH2_RP_ID} OAuth2 RP"
${NETIDM} system oauth2 create "${OAUTH2_RP_ID}" "${OAUTH2_RP_DISPLAY}" "https://localhost:10443" -D "${IDM_ADMIN_USER}"
echo "Creating the ${OAUTH2_RP_ID} OAuth2 RP Scope Map - Group ${TEST_GROUP}"
${NETIDM} system oauth2 update-scope-map "${OAUTH2_RP_ID}" "${TEST_GROUP}" openid -D "${IDM_ADMIN_USER}"
echo "Creating the ${OAUTH2_RP_ID} OAuth2 RP Supplemental Scope Map"
${NETIDM} system oauth2 update-sup-scope-map "${OAUTH2_RP_ID}" "${TEST_GROUP}" admin -D "${IDM_ADMIN_USER}"

echo "Creating a claim map for RS ${OAUTH2_RP_ID}"
${NETIDM} system oauth2 update-claim-map "${OAUTH2_RP_ID}" testclaim "${TEST_GROUP}" foo bar -D "${IDM_ADMIN_USER}"
...
${NETIDM} system oauth2 list -D "${IDM_ADMIN_USER}" | grep -A10 "${OAUTH2_RP_ID}"

# config auth2
echo "Pulling secret for the ${OAUTH2_RP_ID} OAuth2 RP"
OAUTH2_SECRET="$(${NETIDM} system oauth2 show-basic-secret -o json "${OAUTH2_RP_ID}" -D "${IDM_ADMIN_USER}")"
echo "${OAUTH2_SECRET}"

echo "Creating cred reset link for ${TEST_USER_NAME}"
${NETIDM} person credential create-reset-token "${TEST_USER_NAME}" -D "${IDM_ADMIN_USER}"

echo "Done!"

echo "###################################"
echo "idm_admin password: ${IDM_ADMIN_PASS}"
echo "UI URL:             ${NETIDM_URL}"
