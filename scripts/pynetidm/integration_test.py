...
# so we can load netidm without building virtualenvs
sys.path.append("./pynetidm")

from netidm import NetidmClient


def recover_account(username: str) -> str:
    """runs the netidmd binary to recover creds"""
    recover_cmd = [
        "cargo",
        "run",
        "--bin",
...
async def main() -> None:
    ...
    # login time!
    admin_client = NetidmClient(uri=host, ca_path="/tmp/netidm/ca.pem")
    ...
    idm_admin_client = NetidmClient(uri=host, ca_path="/tmp/netidm/ca.pem")
...
if __name__ == "__main__":
    ...
    if not pathlib.Path("scripts/pynetidm/integration_test.py").exists():
