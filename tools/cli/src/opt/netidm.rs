...
#[derive(Debug, Args, Clone)]
pub struct DebugOpt {
    /// Enable debugging of the netidm tool
...
pub struct AccountNamedExpireDateTimeOpt {
    ...
    /// - "now" to expire immediately (this will affect authentication with Netidm, but external systems may not be aware of the change until next time it's validated, typically ~15 minutes)
    datetime: String,
}
...
pub enum Oauth2Opt {
    ...
    /// URL is the target when Netidm redirects the user from the apps listing page.
    ...
    /// The image presented on the Netidm Apps Listing page for an OAuth2 resource server.
    #[clap(name = "set-image")]
    SetImage {
        #[clap(flatten)]
        nopt: Named,
    ...
    /// Enable email-based account linking for an OAuth2 client provider (overrides domain default).
    #[clap(name = "enable-email-link-accounts")]
    EnableEmailLinkAccounts { name: String },
    /// Disable email-based account linking for an OAuth2 client provider (overrides domain default).
    #[clap(name = "disable-email-link-accounts")]
    DisableEmailLinkAccounts { name: String },
    /// Enable email-based account linking globally for all OAuth2 providers (domain default).
    #[clap(name = "enable-domain-email-link-accounts")]
    EnableDomainEmailLinkAccounts,
    /// Disable email-based account linking globally for all OAuth2 providers (domain default).
    #[clap(name = "disable-domain-email-link-accounts")]
    DisableDomainEmailLinkAccounts,
    /// Map a Netidm attribute to a provider claim for JIT provisioning.
    /// netidm-attr must be one of: name, displayname, mail.
    #[clap(name = "set-identity-claim-map")]
    SetIdentityClaimMap {
        name: String,
        #[clap(name = "netidm-attr")]
        netidm_attr: String,
...
pub enum DomainOpt {
    ...
    /// Enable or disable easter eggs in the server. This includes seasonal icons, netidm
...
pub enum SynchOpt {
    ...
    /// and are allowed to be modified by netidm and users. Any attributes not listed in
    ...
    /// a full refresh of the provider is requested. Netidm attributes that have been granted
    ...
    /// the authority of Netidm. This signals the end of a migration from an external IDM into
    /// Netidm. ⚠️  This action can NOT be undone. Once complete, it is most likely
    /// that attempting to recreate a sync account from the same IDM will fail due to conflicting
    /// entries that Netidm now owns.
    #[clap(name = "finalise")]
    Finalise {
        #[clap()]
        account_id: String,
...
pub struct ApiSchemaDownloadOpt {
    /// Where to put the file, defaults to ./netidm-openapi.json
    #[clap(name = "filename", env, default_value = "./netidm-openapi.json")]
    filename: PathBuf,
...
#[clap(about = "Netidm Client Utility")]
pub struct NetidmClientParser {
    #[clap(subcommand)]
    pub commands: NetidmClientOpt,

    /// Enable debugging of the netidm tool
    #[clap(short, long, env = "NETIDM_DEBUG", global = true)]
    pub debug: bool,
    /// Select the instance name you wish to connect to
    #[clap(short = 'I', long = "instance", env = "NETIDM_INSTANCE", global = true,
    value_parser = clap::builder::NonEmptyStringValueParser::new())]
    pub instance: Option<String>,
    /// The URL of the netidm instance
    #[clap(short = 'H', long = "url", env = "NETIDM_URL", global = true,
    value_parser = clap::builder::NonEmptyStringValueParser::new())]
    ...
    /// Log format
    #[clap(short, long = "output", env = "NETIDM_OUTPUT", global = true, default_value=OutputMode::default())]
    output_mode: OutputMode,
    /// Skip hostname verification
    #[clap(
        long = "skip-hostname-verification",
        env = "NETIDM_SKIP_HOSTNAME_VERIFICATION",
    ...
    #[clap(
        ...
        env = "NETIDM_ACCEPT_INVALID_CERTS",
        default_value_t = false,
        global = true
    )]
    accept_invalid_certs: bool,
    /// Path to a file to cache tokens in, defaults to ~/.cache/netidm_tokens
    #[clap(
        short,
        long,
        env = "NETIDM_TOKEN_CACHE_PATH",
    hide = true,
     default_value = None,
    global=true,
    value_parser = clap::builder::NonEmptyStringValueParser::new())]
