...
fn main() {
    ...
    let profile = env::var("NETIDM_BUILD_PROFILE").unwrap_or_else(|_| "developer".to_string());
    ...
    if let Some(commit_rev) = determine_git_rev() {
        println!("cargo:rustc-env=NETIDM_PKG_COMMIT_REV={commit_rev}");
    }

    println!("cargo:rerun-if-changed={}", profile_path.to_string_lossy());

    println!("cargo:rustc-env=NETIDM_BUILD_PROFILE={profile}");
    println!("cargo:rustc-env=NETIDM_BUILD_PROFILE_TOML={contents}");
}
