...
impl NetidmClient {
    ...
    pub async fn wg_tunnel_get(&self, name: &str) -> Result<Option<WgTunnelResponse>, ClientError> {
    ...
    pub async fn wg_peer_list(&self, tunnel: &str) -> Result<Vec<WgPeerResponse>, ClientError> {
        self.perform_get_request(&format!("/v1/wg/tunnel/{tunnel}/peer"))
            .await
    }

    pub async fn wg_peer_delete(&self, tunnel: &str, peer_uuid: &str) -> Result<(), ClientError> {
        self.perform_delete_request(&format!("/v1/wg/tunnel/{tunnel}/peer/{peer_uuid}"))
            .await
    }

    pub async fn wg_token_list(&self, tunnel: &str) -> Result<Vec<WgTokenInfo>, ClientError> {
    ...
    pub async fn wg_token_delete(&self, tunnel: &str, token: &str) -> Result<(), ClientError> {
    ...
    ) -> Result<WgConnectResponse, ClientError> {
