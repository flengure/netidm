...
impl QueryServer {
    ...
    pub async fn initialise_helper(
        ...
        if db_domain_version == 0 {
            ...
            const { assert!(DOMAIN_MIN_CREATION_LEVEL == DOMAIN_LEVEL_40) };
            write_txn.bootstrap_dl40()?;
...
impl QueryServerWriteTransaction<'_> {
    ...
    */
    ...
    pub(crate) fn bootstrap_dl31(&mut self) -> Result<(), OperationError> {
        if !cfg!(test) && DOMAIN_TGT_LEVEL < DOMAIN_LEVEL_31 {
            error!("Unable to raise domain level from 30 to 31.");
            return Err(OperationError::MG0004DomainLevelInDevelopment);
        }

        self.internal_migrate_or_create_batch(
            &format!("phase 1 - schema attrs target {}", DOMAIN_TGT_LEVEL),
            migration_data::dl31::phase_1_schema_attrs(),
        )?;

        self.internal_migrate_or_create_batch(
            "phase 2 - schema classes",
            migration_data::dl31::phase_2_schema_classes(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 3 - key provider",
            migration_data::dl31::phase_3_key_provider(),
        )?;

        self.reload()?;

        self.internal_migrate_or_create_batch(
            "phase 4 - dl31 system entries",
            migration_data::dl31::phase_4_system_entries(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 5 - builtin admin entries",
            migration_data::dl31::phase_5_builtin_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 6 - builtin not admin entries",
            migration_data::dl31::phase_6_builtin_non_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 7 - builtin access control profiles",
            migration_data::dl31::phase_7_builtin_access_control_profiles(),
        )?;

        self.internal_delete_batch(
            "phase 8 - delete UUIDs",
            migration_data::dl31::phase_8_delete_uuids(),
        )?;
    ...
    pub(crate) fn bootstrap_dl32(&mut self) -> Result<(), OperationError> {
        if !cfg!(test) && DOMAIN_TGT_LEVEL < DOMAIN_LEVEL_32 {
            error!("Unable to raise domain level from 31 to 32.");
            return Err(OperationError::MG0004DomainLevelInDevelopment);
        }

        self.internal_migrate_or_create_batch(
            &format!("phase 1 - schema attrs target {}", DOMAIN_TGT_LEVEL),
            migration_data::dl32::phase_1_schema_attrs(),
        )?;

        self.internal_migrate_or_create_batch(
            "phase 2 - schema classes",
            migration_data::dl32::phase_2_schema_classes(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 3 - key provider",
            migration_data::dl32::phase_3_key_provider(),
        )?;

        self.reload()?;

        self.internal_migrate_or_create_batch(
            "phase 4 - dl32 system entries",
            migration_data::dl32::phase_4_system_entries(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 5 - builtin admin entries",
            migration_data::dl32::phase_5_builtin_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 6 - builtin not admin entries",
            migration_data::dl32::phase_6_builtin_non_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 7 - builtin access control profiles",
            migration_data::dl32::phase_7_builtin_access_control_profiles(),
        )?;

        self.internal_delete_batch(
            "phase 8 - delete UUIDs",
            migration_data::dl32::phase_8_delete_uuids(),
        )?;
    ...
    pub(crate) fn bootstrap_dl33(&mut self) -> Result<(), OperationError> {
        if !cfg!(test) && DOMAIN_TGT_LEVEL < DOMAIN_LEVEL_33 {
            error!("Unable to raise domain level from 32 to 33.");
            return Err(OperationError::MG0004DomainLevelInDevelopment);
        }

        self.internal_migrate_or_create_batch(
            &format!("phase 1 - schema attrs target {}", DOMAIN_TGT_LEVEL),
            migration_data::dl33::phase_1_schema_attrs(),
        )?;

        self.internal_migrate_or_create_batch(
            "phase 2 - schema classes",
            migration_data::dl33::phase_2_schema_classes(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 3 - key provider",
            migration_data::dl33::phase_3_key_provider(),
        )?;

        self.reload()?;

        self.internal_migrate_or_create_batch(
            "phase 4 - dl33 system entries",
            migration_data::dl33::phase_4_system_entries(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 5 - builtin admin entries",
            migration_data::dl33::phase_5_builtin_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 6 - builtin not admin entries",
            migration_data::dl33::phase_6_builtin_non_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 7 - builtin access control profiles",
            migration_data::dl33::phase_7_builtin_access_control_profiles(),
        )?;

        self.internal_delete_batch(
            "phase 8 - delete UUIDs",
            migration_data::dl33::phase_8_delete_uuids(),
        )?;
    ...
    pub(crate) fn bootstrap_dl34(&mut self) -> Result<(), OperationError> {
        if !cfg!(test) && DOMAIN_TGT_LEVEL < DOMAIN_LEVEL_34 {
            error!("Unable to raise domain level from 33 to 34.");
            return Err(OperationError::MG0004DomainLevelInDevelopment);
        }

        self.internal_migrate_or_create_batch(
            &format!("phase 1 - schema attrs target {}", DOMAIN_TGT_LEVEL),
            migration_data::dl34::phase_1_schema_attrs(),
        )?;

        self.internal_migrate_or_create_batch(
            "phase 2 - schema classes",
            migration_data::dl34::phase_2_schema_classes(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 3 - key provider",
            migration_data::dl34::phase_3_key_provider(),
        )?;

        self.reload()?;

        self.internal_migrate_or_create_batch(
            "phase 4 - dl34 system entries",
            migration_data::dl34::phase_4_system_entries(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 5 - builtin admin entries",
            migration_data::dl34::phase_5_builtin_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 6 - builtin not admin entries",
            migration_data::dl34::phase_6_builtin_non_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 7 - builtin access control profiles",
            migration_data::dl34::phase_7_builtin_access_control_profiles(),
        )?;

        self.internal_delete_batch(
            "phase 8 - delete UUIDs",
            migration_data::dl34::phase_8_delete_uuids(),
        )?;
    ...
    pub(crate) fn bootstrap_dl35(&mut self) -> Result<(), OperationError> {
        if !cfg!(test) && DOMAIN_TGT_LEVEL < DOMAIN_LEVEL_35 {
            error!("Unable to raise domain level from 34 to 35.");
            return Err(OperationError::MG0004DomainLevelInDevelopment);
        }

        self.internal_migrate_or_create_batch(
            &format!("phase 1 - schema attrs target {}", DOMAIN_TGT_LEVEL),
            migration_data::dl35::phase_1_schema_attrs(),
        )?;

        self.internal_migrate_or_create_batch(
            "phase 2 - schema classes",
            migration_data::dl35::phase_2_schema_classes(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 3 - key provider",
            migration_data::dl35::phase_3_key_provider(),
        )?;

        self.reload()?;

        self.internal_migrate_or_create_batch(
            "phase 4 - dl35 system entries",
            migration_data::dl35::phase_4_system_entries(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 5 - builtin admin entries",
            migration_data::dl35::phase_5_builtin_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 6 - builtin not admin entries",
            migration_data::dl35::phase_6_builtin_non_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 7 - builtin access control profiles",
            migration_data::dl35::phase_7_builtin_access_control_profiles(),
        )?;

        self.internal_delete_batch(
            "phase 8 - delete UUIDs",
            migration_data::dl35::phase_8_delete_uuids(),
        )?;
    ...
    pub(crate) fn bootstrap_dl36(&mut self) -> Result<(), OperationError> {
        if !cfg!(test) && DOMAIN_TGT_LEVEL < DOMAIN_LEVEL_36 {
            error!("Unable to raise domain level from 35 to 36.");
            return Err(OperationError::MG0004DomainLevelInDevelopment);
        }

        self.internal_migrate_or_create_batch(
            &format!("phase 1 - schema attrs target {}", DOMAIN_TGT_LEVEL),
            migration_data::dl36::phase_1_schema_attrs(),
        )?;

        self.internal_migrate_or_create_batch(
            "phase 2 - schema classes",
            migration_data::dl36::phase_2_schema_classes(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 3 - key provider",
            migration_data::dl36::phase_3_key_provider(),
        )?;

        self.reload()?;

        self.internal_migrate_or_create_batch(
            "phase 4 - dl36 system entries",
            migration_data::dl36::phase_4_system_entries(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 5 - builtin admin entries",
            migration_data::dl36::phase_5_builtin_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 6 - builtin not admin entries",
            migration_data::dl36::phase_6_builtin_non_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 7 - builtin access control profiles",
            migration_data::dl36::phase_7_builtin_access_control_profiles(),
        )?;

        self.internal_delete_batch(
            "phase 8 - delete UUIDs",
            migration_data::dl36::phase_8_delete_uuids(),
        )?;
    ...
    pub(crate) fn bootstrap_dl37(&mut self) -> Result<(), OperationError> {
        if !cfg!(test) && DOMAIN_TGT_LEVEL < DOMAIN_LEVEL_37 {
            error!("Unable to raise domain level from 36 to 37.");
            return Err(OperationError::MG0004DomainLevelInDevelopment);
        }

        self.internal_migrate_or_create_batch(
            &format!("phase 1 - schema attrs target {}", DOMAIN_TGT_LEVEL),
            migration_data::dl37::phase_1_schema_attrs(),
        )?;

        self.internal_migrate_or_create_batch(
            "phase 2 - schema classes",
            migration_data::dl37::phase_2_schema_classes(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 3 - key provider",
            migration_data::dl37::phase_3_key_provider(),
        )?;

        self.reload()?;

        self.internal_migrate_or_create_batch(
            "phase 4 - dl37 system entries",
            migration_data::dl37::phase_4_system_entries(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 5 - builtin admin entries",
            migration_data::dl37::phase_5_builtin_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 6 - builtin not admin entries",
            migration_data::dl37::phase_6_builtin_non_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 7 - builtin access control profiles",
            migration_data::dl37::phase_7_builtin_access_control_profiles(),
        )?;

        self.internal_delete_batch(
            "phase 8 - delete UUIDs",
            migration_data::dl37::phase_8_delete_uuids(),
        )?;
    ...
    pub(crate) fn bootstrap_dl38(&mut self) -> Result<(), OperationError> {
        if !cfg!(test) && DOMAIN_TGT_LEVEL < DOMAIN_LEVEL_38 {
            error!("Unable to raise domain level from 37 to 38.");
            return Err(OperationError::MG0004DomainLevelInDevelopment);
        }

        self.internal_migrate_or_create_batch(
            &format!("phase 1 - schema attrs target {}", DOMAIN_TGT_LEVEL),
            migration_data::dl38::phase_1_schema_attrs(),
        )?;

        self.internal_migrate_or_create_batch(
            "phase 2 - schema classes",
            migration_data::dl38::phase_2_schema_classes(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 3 - key provider",
            migration_data::dl38::phase_3_key_provider(),
        )?;

        self.reload()?;

        self.internal_migrate_or_create_batch(
            "phase 4 - dl38 system entries",
            migration_data::dl38::phase_4_system_entries(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 5 - builtin admin entries",
            migration_data::dl38::phase_5_builtin_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 6 - builtin not admin entries",
            migration_data::dl38::phase_6_builtin_non_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 7 - builtin access control profiles",
            migration_data::dl38::phase_7_builtin_access_control_profiles(),
        )?;

        self.internal_delete_batch(
            "phase 8 - delete UUIDs",
            migration_data::dl38::phase_8_delete_uuids(),
        )?;
    ...
    pub(crate) fn bootstrap_dl39(&mut self) -> Result<(), OperationError> {
        if !cfg!(test) && DOMAIN_TGT_LEVEL < DOMAIN_LEVEL_39 {
            error!("Unable to raise domain level from 38 to 39.");
            return Err(OperationError::MG0004DomainLevelInDevelopment);
        }

        self.internal_migrate_or_create_batch(
            &format!("phase 1 - schema attrs target {}", DOMAIN_TGT_LEVEL),
            migration_data::dl39::phase_1_schema_attrs(),
        )?;

        self.internal_migrate_or_create_batch(
            "phase 2 - schema classes",
            migration_data::dl39::phase_2_schema_classes(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 3 - key provider",
            migration_data::dl39::phase_3_key_provider(),
        )?;

        self.reload()?;

        self.internal_migrate_or_create_batch(
            "phase 4 - dl39 system entries",
            migration_data::dl39::phase_4_system_entries(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 5 - builtin admin entries",
            migration_data::dl39::phase_5_builtin_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 6 - builtin not admin entries",
            migration_data::dl39::phase_6_builtin_non_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 7 - builtin access control profiles",
            migration_data::dl39::phase_7_builtin_access_control_profiles(),
        )?;

        self.internal_delete_batch(
            "phase 8 - delete UUIDs",
            migration_data::dl39::phase_8_delete_uuids(),
        )?;
    ...
    pub(crate) fn bootstrap_dl40(&mut self) -> Result<(), OperationError> {
        if !cfg!(test) && DOMAIN_TGT_LEVEL < DOMAIN_LEVEL_40 {
            error!("Unable to raise domain level from 39 to 40.");
            return Err(OperationError::MG0004DomainLevelInDevelopment);
        }

        self.internal_migrate_or_create_batch(
            &format!("phase 1 - schema attrs target {}", DOMAIN_TGT_LEVEL),
            migration_data::dl40::phase_1_schema_attrs(),
        )?;

        self.internal_migrate_or_create_batch(
            "phase 2 - schema classes",
            migration_data::dl40::phase_2_schema_classes(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 3 - key provider",
            migration_data::dl40::phase_3_key_provider(),
        )?;

        self.reload()?;

        self.internal_migrate_or_create_batch(
            "phase 4 - dl40 system entries",
            migration_data::dl40::phase_4_system_entries(),
        )?;
        ...
        self.internal_migrate_or_create_batch(
            "phase 5 - builtin admin entries",
            migration_data::dl40::phase_5_builtin_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 6 - builtin not admin entries",
            migration_data::dl40::phase_6_builtin_non_admin_entries()?,
        )?;

        self.internal_migrate_or_create_batch(
            "phase 7 - builtin access control profiles",
            migration_data::dl40::phase_7_builtin_access_control_profiles(),
        )?;

        self.internal_delete_batch(
            "phase 8 - delete UUIDs",
            migration_data::dl40::phase_8_delete_uuids(),
        )?;
    ...
    async fn test_migrations_dl28_idempotent(server: &QueryServer) {
        ...
        write_txn
            .internal_create(vec![entry_init!(
                ...
                (Attribute::ConnectorId, Value::new_utf8s("github-client-id")),
                (
                ...
                (Attribute::ConnectorProviderKind, Value::new_iutf8("github")),
                (
                ...
                (Attribute::ConnectorGithubLoadAllGroups, Value::Bool(true)),
                (
