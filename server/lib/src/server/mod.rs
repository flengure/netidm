...
impl<'a> QueryServerWriteTransaction<'a> {
    ...
    pub(crate) fn reload_domain_info_version(&mut self) -> Result<(), OperationError> {
        ...
        */
        ...
        const { assert!(DOMAIN_MAX_LEVEL == DOMAIN_LEVEL_40) };
        ...
        if previous_version < DOMAIN_LEVEL_29 && domain_info_version == DOMAIN_LEVEL_29 {
            self.bootstrap_dl29()?;
        }

        if previous_version < DOMAIN_LEVEL_30 && domain_info_version == DOMAIN_LEVEL_30 {
            self.bootstrap_dl30()?;
        }

        if previous_version < DOMAIN_LEVEL_31 && domain_info_version == DOMAIN_LEVEL_31 {
            self.bootstrap_dl31()?;
        }
        ...
        if previous_version < DOMAIN_LEVEL_34 && domain_info_version == DOMAIN_LEVEL_34 {
            self.bootstrap_dl34()?;
        }

        if previous_version < DOMAIN_LEVEL_35 && domain_info_version == DOMAIN_LEVEL_35 {
            self.bootstrap_dl35()?;
        }

        if previous_version < DOMAIN_LEVEL_36 && domain_info_version == DOMAIN_LEVEL_36 {
            self.bootstrap_dl36()?;
        }

        if previous_version < DOMAIN_LEVEL_37 && domain_info_version == DOMAIN_LEVEL_37 {
            self.bootstrap_dl37()?;
        }

        if previous_version < DOMAIN_LEVEL_38 && domain_info_version == DOMAIN_LEVEL_38 {
            self.bootstrap_dl38()?;
        }

        if previous_version < DOMAIN_LEVEL_39 && domain_info_version == DOMAIN_LEVEL_39 {
            self.bootstrap_dl39()?;
        }

        if previous_version < DOMAIN_LEVEL_40 && domain_info_version == DOMAIN_LEVEL_40 {
            self.bootstrap_dl40()?;
        }
