...
mod tests {
    ...
    fn test_idm_authsession_oauth2_client() {
        ...
        // GenericOidc now dispatches directly to the connector registry.
        // The HTTP handler receives GenericOidcCallbackRequest and calls
        // GenericOidcConnector::fetch_callback_claims; no further AuthSession
        // steps occur on this code path.
        let AuthState::External(AuthExternal::GenericOidcCallbackRequest {
            code,
            code_verifier: _,
            provider_uuid,
            email_link_accounts: _,
        }) = state
        else {
            unreachable!("Expected GenericOidcCallbackRequest, got {state:?}")
        };
