...
//!   * Four Google-specific config attributes on `EntryClass::OAuth2Client`
...
pub fn phase_1_schema_attrs() -> Vec<EntryInitNew> {
    ...
    attrs.push(
        SCHEMA_ATTR_OAUTH2_CLIENT_GOOGLE_HOSTED_DOMAIN_DL30
            .clone()
            .into(),
    );
    attrs.push(
        SCHEMA_ATTR_OAUTH2_CLIENT_GOOGLE_SERVICE_ACCOUNT_JSON_DL30
            .clone()
            .into(),
    );
    attrs.push(
        SCHEMA_ATTR_OAUTH2_CLIENT_GOOGLE_ADMIN_EMAIL_DL30
            .clone()
            .into(),
    );
    attrs.push(
        SCHEMA_ATTR_OAUTH2_CLIENT_GOOGLE_FETCH_GROUPS_DL30
            .clone()
            .into(),
    );
    attrs
