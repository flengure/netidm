...
//!   * Five new optional config attributes on `EntryClass::Connector`:
...
pub fn phase_1_schema_attrs() -> Vec<EntryInitNew> {
    ...
    attrs.push(
        SCHEMA_ATTR_CONNECTOR_GITLAB_GET_GROUPS_PERMISSION_DL35
            .clone()
            .into(),
    );
    attrs.push(SCHEMA_ATTR_CONNECTOR_GITLAB_ROOT_CA_DL35.clone().into());
    attrs
}
