...
//!   * One discriminator attribute (`ConnectorProviderKind`) selecting
//!     which concrete connector implementation handles a given
//!     `Connector` entry. Value `"github"` in this PR; absence
//!     defaults to `"generic-oidc"` at the dispatch site.
//!   * Seven GitHub-specific config attributes on `EntryClass::Connector`
...
pub(crate) use self::schema::SCHEMA_ATTR_CONNECTOR_PROVIDER_KIND_DL28;
...
pub fn phase_1_schema_attrs() -> Vec<EntryInitNew> {
    ...
    attrs.push(
        SCHEMA_ATTR_CONNECTOR_GITHUB_TEAM_NAME_FIELD_DL28
            .clone()
            .into(),
    );
    attrs.push(
        SCHEMA_ATTR_CONNECTOR_GITHUB_LOAD_ALL_GROUPS_DL28
            .clone()
            .into(),
    );
    attrs.push(
        SCHEMA_ATTR_CONNECTOR_GITHUB_PREFERRED_EMAIL_DOMAIN_DL28
            .clone()
            .into(),
    );
    attrs.push(
        SCHEMA_ATTR_CONNECTOR_GITHUB_ALLOW_JIT_PROVISIONING_DL28
            .clone()
            .into(),
    );
    attrs
...
pub fn phase_2_schema_classes() -> Vec<EntryInitNew> {
    // DL28 adds no NEW classes, but extends the existing `Connector`
