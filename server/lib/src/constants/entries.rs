...
pub use netidm_proto::attribute::Attribute;
...
impl From<EntryClass> for &'static str {
    fn from(val: EntryClass) -> Self {
        match val {
            ...
            EntryClass::Connector => ENTRYCLASS_CONNECTOR,
            EntryClass::SamlClient => ENTRYCLASS_SAML_CLIENT,
            EntryClass::LogoutDelivery => ENTRYCLASS_LOGOUT_DELIVERY,
            EntryClass::SamlSession => ENTRYCLASS_SAML_SESSION,
            EntryClass::ProviderIdentity => ENTRYCLASS_PROVIDER_IDENTITY,
            EntryClass::WgTunnel => ENTRYCLASS_WG_TUNNEL,
            EntryClass::WgPeer => ENTRYCLASS_WG_PEER,
            EntryClass::WgToken => ENTRYCLASS_WG_TOKEN,
