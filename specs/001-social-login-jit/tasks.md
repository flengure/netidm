...
- [X] T051 Validate the full quickstart.md scenario end-to-end: GitHub happy path, returning user, JIT disabled denial, username collision (depends on T048). Covered by `#[idm_test]` integration tests: `test_link_or_provision_chain_jit_creates_new_account`, `test_link_or_provision_chain_returning_user_no_duplicate`, `test_link_or_provision_chain_username_collision_appends_suffix`, `test_jit_disabled_rejects_unknown_user` in `server/lib/src/idm/connector/github.rs`; `test_jit_provision_oauth2_account_happy_path`, `test_jit_provision_oauth2_account_returning_user_no_duplicate`, `test_derive_jit_username_collision_appends_suffix` in `server/lib/src/idm/server.rs` (manual `/ui/login/provision` path for Google/non-GitHub-connector providers). Live-browser validation against a real GitHub/Google OAuth app is out of scope for automation.
