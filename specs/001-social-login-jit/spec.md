...
**Created**: 2026-04-16
**Status**: Draft
**Input**: User description: "Social Login with JIT Provisioning — Add GitHub and Google as social login providers to Kanidm, with Just-In-Time account provisioning on first login."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - First-Time Social Login (Priority: P1)

A new user who has never logged into Kanidm visits an application that delegates authentication to Kanidm. They click "Sign in with GitHub" (or Google), are redirected to the provider, authenticate successfully, and are automatically given a Kanidm account — without any prior admin setup for that individual. They are then logged in and redirected back to the application, all within a single login flow.
...
**Independent Test**: Can be fully tested by configuring a Kanidm OAuth2 provider with JIT enabled and logging in as a brand-new user — no prior account creation needed. Delivers a working end-to-end social login flow.

**Acceptance Scenarios**:

1. **Given** a Kanidm instance with GitHub configured as a social login provider and JIT provisioning enabled, **When** a user with no existing Kanidm account authenticates via GitHub for the first time, **Then** a new Kanidm account is created automatically and the user is logged in.
2. **Given** a Kanidm instance with JIT provisioning disabled, **When** a user with no existing account authenticates via a social provider, **Then** login is denied with a clear error message and no account is created.
...
A Kanidm administrator wants to enable GitHub or Google as a login option. They use the Kanidm CLI to register the provider (supplying client ID and secret), enable JIT provisioning, and optionally map provider claims to Kanidm account fields. No code changes or restarts are required.
...
**Independent Test**: Can be tested by running CLI commands to create a GitHub/Google provider configuration, then verifying the provider appears in Kanidm and the login UI shows the provider option.

**Acceptance Scenarios**:

1. **Given** a Kanidm administrator with appropriate permissions, **When** they run `kanidm system oauth2-client create-github <name> <client_id> <client_secret>`, **Then** a GitHub provider is registered with correct default endpoints and required scopes.
2. **Given** a registered social provider, **When** the admin runs `kanidm system oauth2-client enable-jit-provisioning <name>`, **Then** JIT provisioning is enabled for that provider and new users can be auto-provisioned on first login.
3. **Given** a registered social provider, **When** the admin maps a claim (`set-claim-map <name> displayname name`), **Then** the user's display name on their Kanidm account is populated from the provider's `name` claim on first login.

---

### User Story 3 - New Account Review Before Activation (Priority: P3)

A new user authenticating via a social provider for the first time is shown a confirmation page before their Kanidm account is created. The page displays the proposed username, display name, and email derived from the provider's claims. The user can edit the proposed username before confirming. Only after confirmation is the account created.
...
- What happens when the derived username collides with an existing Kanidm account name?
- What happens when the provider's user ID (`sub`) is already linked to a different Kanidm account?
...
- **FR-004**: Administrators MUST be able to map provider-supplied identity claims (e.g. name, email, username) to Kanidm account fields on a per-provider basis.
- **FR-005**: When a user authenticates via a social provider for the first time and JIT provisioning is enabled, the system MUST automatically create a Kanidm account for that user without administrator intervention.
...
- **FR-007**: Each social provider MUST be associated with a unique identifier per user (a stable subject identifier from the provider), and subsequent logins MUST resolve to the same Kanidm account using this identifier.
...
- **FR-014**: All administrator configuration of social providers MUST be available via the Kanidm CLI.
...
- **Provider Claim Mapping**: Maps a provider claim name (e.g. `name`) to a Kanidm account attribute (e.g. display name). Belongs to a Social Provider Configuration.
- **Linked Provider Identity**: Links a Kanidm account to an external provider. Attributes: provider reference, stable provider subject identifier, credential UUID. Ensures a user can be looked up by provider + subject on subsequent logins.
- **Kanidm Account**: The automatically-provisioned user account. Attributes derived from provider claims: username (iname), display name, email. Created only once per unique provider subject identifier.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: A brand-new user can complete social login and have a Kanidm account created within the same login flow, with no prior administrator action for that specific user.
...
- A single Kanidm account can only be linked to one external provider identity at a time in v1 (no multi-provider linking or email-based account merging).
- Profile data from the provider (display name, email) is used only at account creation time; subsequent changes at the provider are not synced back to Kanidm.
- Email is treated as informational at provisioning time; a provider returning no email does not block account creation, but the email field on the Kanidm account will be empty.
- The administrator performing provider configuration holds the appropriate Kanidm administrative role.
- Kanidm's existing session and redirect handling is reused after account creation; no new session infrastructure is required.
