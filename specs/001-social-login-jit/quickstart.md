...
- Netidm server running locally with admin access
- GitHub OAuth App created at https://github.com/settings/developers
  - Callback URL: `https://<netidm-host>/ui/login/oauth2_landing`
- OR Google OAuth2 credentials from Google Cloud Console
  - Authorized redirect URI: `https://<netidm-host>/ui/login/oauth2_landing`
...
netidm system oauth2 create-github mygithub <CLIENT_ID> <CLIENT_SECRET> -H https://localhost:8443 -D admin

# 2. Add the callback URL
netidm system oauth2 add-redirect-url mygithub https://localhost:8443/ui/login/oauth2_landing \
  -H https://localhost:8443 -D admin

# 3. Enable JIT provisioning
netidm system oauth2 enable-jit-provisioning mygithub -H https://localhost:8443 -D admin

# 4. Verify configuration
netidm system oauth2 get mygithub -H https://localhost:8443 -D admin
...
```bash
netidm system oauth2 create-google mygoogle <CLIENT_ID> <CLIENT_SECRET> -H https://localhost:8443 -D admin
netidm system oauth2 add-redirect-url mygoogle https://localhost:8443/ui/login/oauth2_landing \
  -H https://localhost:8443 -D admin
netidm system oauth2 enable-jit-provisioning mygoogle -H https://localhost:8443 -D admin
```
...
6. **Expect**: Redirected to the Netidm home/app page — logged in
7. Verify account was created:
   ```bash
   netidm account get <proposed-username> -H https://localhost:8443 -D admin
...
```bash
netidm system oauth2 disable-jit-provisioning mygithub -H https://localhost:8443 -D admin
```
...
1. Pre-create an account with the same username as your GitHub `login`:
   ```bash
   netidm person create <your-github-login> "Test Collision" -H https://localhost:8443 -D admin
   ```
