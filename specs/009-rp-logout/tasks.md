...
- [X] T016 [US1] Replace the placeholder in `server/lib/src/idm/logout.rs` with full module: module-level `//!` doc; `pub struct LogoutTokenClaims { iss, aud, iat, jti, sub, sid, events }`; `pub fn logout_token_for_rp(idms, rp_uuid, user_uuid, session_uuid) -> Result<String, OperationError>` that builds the claims per `research.md` R2 and signs with `compact_jwt::Jws`; include `typ: "logout+jwt"` header and match the RP's registered ID-token signing `alg`; `# Errors` on every `Result`-returning `pub fn` — landed as `IdmServerProxyWriteTransaction::enqueue_backchannel_logout_for_rp` in `server/lib/src/idm/oauth2.rs` (co-located with the RS signing material it needs)
...
- [X] T018 [US1] Add unit test `logout_token_claims_round_trip` in `server/lib/src/idm/logout.rs`: build a `LogoutTokenClaims`, sign, parse via `compact_jwt::OidcUnverified`, assert every claim round-trips including the literal `events` map shape and the `typ` header
- [X] T019 [US1] Add unit test `terminate_session_revokes_refresh_tokens` in `server/lib/src/idm/logout.rs`: seed a test DB with a UAT and a linked OAuth2 refresh token, call `terminate_session`, assert the refresh token is no longer accepted by the token endpoint — landed in `idm::oauth2::tests` alongside `setup_refresh_token`
...
- [X] T022 [US1] Create askama template `server/core/templates/logged_out.html` per `research.md` R8: heading, one-paragraph body, link to netidm UI home; inherits existing base template; no branding additions — shipped as inline HTML in `end_session_confirmation_response()` in `server/core/src/https/oauth2.rs`; no separate template file (one-off page, not worth the askama indirection)
- [X] T023 [US1] Create `server/core/src/https/views/logout.rs` that renders `logged_out.html` (askama `Template` derive); export `LoggedOutPage` for reuse by the route layer — subsumed by T022's inline response; no separate view module needed
...
- [-] T055 (deferred) [P] [US3] Integration test `backchannel_logout_delivery_resumes_after_restart` in `server/testkit/tests/`: enqueue a pending delivery; gracefully restart the testkit server; on next boot, assert the worker picks up the pending record and transitions it to `Succeeded` once the receiver comes online (SC-008). **Deferred**: the current testkit spins up a fresh in-memory server per `#[netidmd_testkit::test]` call — there is no shared file-backed DB and no shutdown-then-reopen primitive. Implementing this test requires new testkit scaffolding (persistent DB path + `CoreHandle::shutdown().await` that waits for all spawned tasks + second `create_server_core` against the same DB), which is out of scope for this PR. The lifecycle property is partially protected by the unit-level `test_idm_logout_terminate_session_revokes_refresh_tokens` and `logout_delivery_state_transitions` tests (which cover the state machine the worker resumes against); full end-to-end restart resilience is a follow-up PR.
