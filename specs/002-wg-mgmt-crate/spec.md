...
**Independent Test**: Create a WgTunnel entry in Kanidm and start the daemon. Verify the interface appears on the host, has the correct address, and the WireGuard configuration is active.

**Acceptance Scenarios**:

1. **Given** a WgTunnel entry exists in Kanidm with a private key, listen port, address, and endpoint, **When** kanidmd starts, **Then** the corresponding network interface is created, configured, and brought up within 5 seconds.
...
**Independent Test**: Issue a registration token scoped to a tunnel. Present the token and a generated public key to the registration endpoint. Verify a WgPeer entry is created in Kanidm, an IP is allocated, the peer is hot-added to the live interface, and the returned config contains correct server details and the peer's assigned address.
...
**Independent Test**: Bring up a tunnel with at least one registered peer. Wait for the background polling cycle. Verify that the WgPeer entry in Kanidm has a `last_seen` timestamp updated within 60 seconds of a completed handshake.

**Acceptance Scenarios**:

1. **Given** a registered peer has completed a WireGuard handshake, **When** the background poller runs (at most every 30 seconds), **Then** the corresponding WgPeer entry in Kanidm is updated with the correct last-handshake timestamp.
...
An administrator wants to immediately remove a peer from an active WireGuard tunnel by deleting the corresponding entry in Kanidm, without restarting the tunnel or interrupting other peers.

**Why this priority**: Security hygiene. Peer removal must be hot-applied so that access revocation takes effect immediately.

**Independent Test**: Register a peer, confirm the handshake succeeds, then delete the WgPeer entry in Kanidm. Verify the peer is removed from the live WireGuard interface within the next watch/poll cycle.

**Acceptance Scenarios**:

1. **Given** a WgPeer entry is deleted in Kanidm, **When** the daemon detects the deletion, **Then** the peer is removed from the live interface within 30 seconds.
...
- **FR-002**: The system MUST bring up all enabled WgTunnel entries found in Kanidm on daemon startup, creating network interfaces and applying configurations derived from those entries.
...
- **FR-011**: The system MUST poll all active WireGuard interfaces at most every 30 seconds and write the latest handshake timestamp for each peer back to its WgPeer entry in Kanidm.
- **FR-012**: The system MUST hot-remove a peer from the live WireGuard interface within 30 seconds of its WgPeer entry being deleted or suspended in Kanidm.
...
- **SC-003**: Peer handshake timestamps in Kanidm are never more than 60 seconds stale relative to the last completed handshake.
- **SC-004**: A peer revoked in Kanidm is removed from the live interface within 30 seconds, with zero disruption to other active peers.
...
- The DL16 schema migration (WgTunnel and WgPeer entry classes) is already present in the Kanidm database before this feature is activated.
...
- Registration token issuance is a separate admin operation (existing Kanidm CLI/API); this feature only consumes tokens.
