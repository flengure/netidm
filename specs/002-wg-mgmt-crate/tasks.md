...
**Goal**: kanidmd reads all enabled WgTunnel entries at startup, detects kernel vs userspace, creates live interfaces.

**Independent Test**: Create a WgTunnel entry, start kanidmd on a Linux host. Verify `ip link show wg0` shows the interface with correct address and WireGuard config (`wg show wg0`).
...
- [X] T018 [US1] Integrate `WgManager` into kanidmd startup in `server/core/src/lib.rs` (or equivalent startup path): call `detect_backend()`, construct `WgManager`, read all WgTunnel entries via `IdmServer`, call `bring_up` for each enabled tunnel; store `Arc<WgManager>` in `ServerState`
...
- [X] T022 [P] [US1] Add `WgOpt` enum to `tools/cli/src/opt/kanidm.rs` with subcommands: `TunnelCreate`, `TunnelGet`, `TunnelList`, `TunnelPatch`, `TunnelDelete`; add `wg` subcommand to root opt
- [X] T023 [US1] Implement CLI dispatch in `tools/cli/src/cli/wg.rs` for all tunnel subcommands; wire into main CLI dispatch

**Checkpoint**: US1 complete — `kanidmd` brings up WireGuard tunnels from Kanidm entries on both kernel and userspace hosts.
...
**Independent Test**: Create a WgTunnel + WgToken (1 use). POST `/v1/wg/connect` with valid Kanidm bearer token, WG token secret, and generated pubkey. Verify WgPeer entry created in Kanidm, peer appears in `wg show wg0 peers`, returned config is valid wg-quick format.
...
- [X] T030 [P] [US2] Add `WgOpt` token subcommands to `tools/cli/src/opt/kanidm.rs`: `TokenCreate { tunnel, uses, expiry, principal }`, `TokenList { tunnel }`, `TokenDelete { tunnel, token_name }`
...
**Checkpoint**: US3 complete — `WgLastSeen` updated in Kanidm within 60s of a peer handshake.

---

## Phase 6: User Story 4 — Peer Revocation (Priority: P4)

**Goal**: Deleting a WgPeer entry in Kanidm causes the daemon to hot-remove the peer from the live interface within 30 seconds.
...
- [X] T037 [US4] Add `QueryServerWriteV1::handle_wg_peer_delete` to `server/core/src/actors/v1_write.rs`; deletes WgPeer entry in Kanidm
- [X] T038 [US4] Implement peer revocation watch in `server/core/src/lib.rs`: 30s background task diffs live WG peers vs Kanidm DB, calls `wg_manager.remove_peer()` for orphaned pubkeys
...
- [X] T040 [P] [US4] Add `WgOpt` peer subcommands to `tools/cli/src/opt/kanidm.rs`: `PeerList { tunnel }`, `PeerDelete { tunnel, peer_uuid }`; implement dispatch in `tools/cli/src/cli/wg.rs`
...
- [X] T041 [P] `cargo clippy -- -D warnings` passes clean across kanidmd_lib, kanidmd_core, kanidmd_wg, kanidm_tools, kanidm_client — no `#[allow(...)]` bypasses
...
4. **Validate**: start kanidmd, check `ip link show wg0` and `wg show wg0`
