"""Generate the OpenAPI client package for pynetidm.
...
DEFAULT_PACKAGE = "netidm_openapi_client"
...
DEFAULT_CA_PATH_ENV = os.getenv("NETIDM_CA_PATH")
...
def _load_docker_module() -> Any:
    try:
        ...
        raise RuntimeError(
            "OpenAPI codegen requires the optional feature 'openapi_codegen'. Install with: pip install 'netidm[openapi_codegen]'"
        ) from exc
...
def _get_project_version() -> str:
    ...
    try:
        ...
        return importlib.metadata.version("netidm")
...
def _run_generator(
    ...
    additional_properties = [
        ...
        f"httpUserAgent=netidm-python/{_get_project_version()}",
    ]
    if library:
    ...
    notice.write_text(
        "\n".join(
            [
                ...
                "Generator: netidm_openapi_codegen",
                "",
                "To regenerate:",
                "  uv run netidm_openapi_codegen",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate the OpenAPI client package for pynetidm.")
    parser.add_argument(
        "--spec-url",
        default=os.getenv("NETIDM_OPENAPI_SPEC_URL", DEFAULT_SPEC_URL),
    ...
    parser.add_argument(
        ...
        help="CA file path for TLS verification (default: NETIDM_CA_PATH if set).",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--generator-image",
        default=os.getenv("NETIDM_OPENAPI_GENERATOR_IMAGE", DEFAULT_GENERATOR_IMAGE),
        help=f"OpenAPI generator Docker image (default: {DEFAULT_GENERATOR_IMAGE}).",
    )
    parser.add_argument(
        "--library",
        default=os.getenv("NETIDM_OPENAPI_LIBRARY", DEFAULT_LIBRARY),
