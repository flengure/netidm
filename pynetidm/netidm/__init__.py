...
from netidm_openapi_client.exceptions import ApiException as OpenApiException
...
from netidm_openapi_client.models.entry import Entry as OpenApiEntry
from pydantic import ValidationError
import yarl

from netidm.models.group import Group, GroupList, RawGroup
from netidm.models.oauth2_rs import OAuth2Rs, Oauth2RsList, RawOAuth2Rs
from netidm.models.person import (
    Person,
    PersonList,
    RawPerson,
    PersonCredentialResetToken,
)
from netidm.models.service_account import (
    ServiceAccount,
    RawServiceAccount,
    ServiceAccountList,
)
...
if TYPE_CHECKING:
    from netidm_openapi_client.models.scim_entry import ScimEntry as OpenApiScimEntry
    from netidm_openapi_client.models.scim_list_response import ScimListResponse as OpenApiScimListResponse

K_AUTH_SESSION_ID = "x-netidm-auth-session-id"

XDG_CACHE_HOME = (
    Path(os.getenv("LOCALAPPDATA", "~/AppData/Local")) / "cache" if platform.system() == "Windows" else Path(os.getenv("XDG_CACHE_HOME", "~/.cache"))
)

TOKEN_PATH = XDG_CACHE_HOME / "netidm_tokens"


class NetidmClient:
    """Netidm client module

    config: a `NetidmClientConfig` object, if this is set, everything else is ignored
    ...
    def __init__(
        ...
        config: Optional[NetidmClientConfig] = None,
        ...
        if config is not None:
            ...
            self.config = NetidmClientConfig.model_validate(
                {
                    "uri": uri,
    ...
    def _normalise_openapi_data(data: Any) -> Any:
        if isinstance(data, list):
            return [NetidmClient._normalise_openapi_data(item) for item in data]
        if isinstance(data, tuple):
            return tuple(NetidmClient._normalise_openapi_data(item) for item in data)
        if isinstance(data, dict):
            return {key: NetidmClient._normalise_openapi_data(value) for key, value in data.items()}
        if hasattr(data, "to_dict") and callable(getattr(data, "to_dict")):
            return NetidmClient._normalise_openapi_data(data.to_dict())
        return data
    ...
    async def get_radius_token(self, username: str) -> ClientResponse[Any]:
        ...
        if response.status_code == 404:
            opid = self._header_value(response.headers, "x-netidm-opid") or ""
