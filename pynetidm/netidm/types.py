...
class AuthBeginResponse(BaseModel):
    ...
    # this should be pulled from the response headers as x-netidm-auth-session-id
...
class RadiusClient(BaseModel):
    """Client config for Netidm FreeRADIUS integration,
...
class NetidmClientConfig(BaseModel):
    """Configuration file definition for Netidm client config
    Based on struct NetidmClientConfig in netidm_client/src/lib.rs
