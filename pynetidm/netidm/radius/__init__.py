...
from netidm.exceptions import NoMatchingEntries
from netidm.types import AuthState, RadiusTokenResponse
...
CONFIG_PATHS = [
    os.getenv("NETIDM_RLM_CONFIG", CONTAINER_CONFIG_FILE_PATH),  # container goodness
    "~/.config/radius.toml",  # for a user
    "/etc/netidm/radius.toml",  # system-wide
    "../examples/radius.toml",  # test mode
    "/data/netidm",  # fallback to old path
]
...
def instantiate(_: Any) -> Any:
    ...
    if config_path is None:
        ...
        netidm_client = NetidmClient(config_file=config_path)
        if netidm_client.config.auth_token is None:
            logging.error("You need to specify auth_token in the configuration file!")
            sys.exit(1)
        os.environ["NETIDM_CONFIG_FILE"] = config_path.as_posix()
        logging.info("Config file: %s", os.environ["NETIDM_CONFIG_FILE"])
...
async def _get_radius_token(
    ...
    response = await netidm_client.get_radius_token(username=username)
...
def authorize(
    ...
    """does the netidm authorize step"""
    logging.info("netidm python module called")
    netidm_client = NetidmClient(config_file=os.environ["NETIDM_CONFIG_FILE"])
    ...
    try:
        ...
        logging.info(
            "netidm RLM_MODULE_NOTFOUND after NoMatchingEntries for user_id %s: %s",
            user_id,
            error_message,
        )
        return radiusd.RLM_MODULE_NOTFOUND
    except ClientConnectorError as client_error:
        logging.error("netidm client connector error in http layer: %s", client_error)
        return radiusd.RLM_MODULE_FAIL
    except Exception as error_message:  # pylint: disable=broad-except
        logging.error("netidm exception: %s, %s", type(error_message), error_message)
        return radiusd.RLM_MODULE_FAIL
    if tok is None:
        logging.info("netidm RLM_MODULE_REJECT - unable to retrieve radius information token")
    ...
    req_sat = False
    required_groups = netidm_client.config.radius_required_groups
    ...
    uservlan: int = reduce(
        check_vlan,
        tok.groups,
        netidm_client.config.radius_default_vlan,
    )
    if uservlan == int(0):
    ...
    reply = (
        ("User-Name", str(name)),
        ("Reply-Message", f"Netidm-Uuid: {uuid}"),
        ("Tunnel-Type", "13"),
...
def authenticate(
    ...
    """authenticate the RADIUS service account to Netidm"""
    netidm_client = NetidmClient(config_file=os.environ["NETIDM_CONFIG_FILE"])
    logging.error("authenticate - %s:%s", acct, password)

    try:
        loop = asyncio.get_event_loop()
        return loop.run_until_complete(netidm_client.check_token_valid())
    except Exception as error_message:  # pylint: disable=broad-except
        logging.error("Failed to run netidm.check_token_valid: %s", error_message)
    return radiusd.RLM_MODULE_FAIL
