...
# This sets up a Netidm environment for doing RADIUS testing.
...
export NETIDM_URL="https://localhost:8443"
export NETIDM_CA_PATH="/tmp/netidm/ca.pem"

cd ../../ || exit 1

echo "Logging in as admin"
cargo run --bin netidm -- login --name admin --password "${ADMIN}"

echo "Logging in as idm_admin"
cargo run --bin netidm -- login --name idm_admin --password "${IDM_ADMIN}"

echo "Creating person 'test'"
cargo run --bin netidm -- person create test test --name idm_admin

echo "Creating group 'radius_access_allowed'"
cargo run --bin netidm -- group create radius_access_allowed --name idm_admin
echo "Adding 'test' to group 'radius_access_allowed'"
cargo run --bin netidm -- group add-members radius_access_allowed test --name idm_admin

echo "Creating radius secret for 'test'"
cargo run --bin netidm -- person radius generate-secret test --name idm_admin
echo "Showing radius secret for 'test'"
cargo run --bin netidm -- person radius show-secret test --name idm_admin
...
cargo run --bin netidm -- service-account create radius_server radius_server --name idm_admin

echo "Setting radius_server to be allowed to be a RADIUS server"
cargo run --bin netidm group add-members --name admin idm_radius_servers radius_server

echo "Creating API Token for 'radius_server' account"
cargo run --bin netidm -- service-account  api-token generate radius_server radius --name admin
