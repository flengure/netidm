...
from .testutils import client, NETIDM_IDM_ADMIN, openapi_ca_path, openapi_server_url, openapi_verify_tls
from netidm import NetidmClient

logging.basicConfig(level=logging.DEBUG)

RADIUS_TEST_USER = os.getenv("NETIDM_RADIUS_TEST_USER")
...
async def radius_token_client(
...
) -> AsyncIterator[NetidmClient]:
    """Client fixture with a usable auth token for radius tests.

    Preference order:
    1. Existing valid auth_token from ~/.config/netidm
    2. Generate a token from IDM_ADMIN_PASS and write a temporary config file
    """
    cleanup_clients: list[NetidmClient] = []
    local_config = Path("~/.config/netidm").expanduser()

    if local_config.exists():
        local_client = NetidmClient(config_file=local_config)
    ...
    if not admin_password:
        pytest.skip("Need either a valid ~/.config/netidm auth_token or IDM_ADMIN_PASS for radius token tests")  # type: ignore[call-non-callable]

    token_source_client = NetidmClient(
...
@pytest.mark.network
@pytest.mark.asyncio
async def test_radius_call(radius_token_client: NetidmClient) -> None:
    """tests the radius call step"""
    test_user = RADIUS_TEST_USER or radius_token_client.config.username or NETIDM_IDM_ADMIN
