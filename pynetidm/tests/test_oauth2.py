...
@pytest.mark.network
@pytest.mark.asyncio
async def test_oauth2_rs_list(client: NetidmClient) -> None:
    ...
    password = os.getenv("NETIDM_PASSWORD")
    if password is None:
        print("No NETIDM_PASSWORD env var set for testing")
        pytest.skip("No NETIDM_PASSWORD env var set for testing")  # type: ignore[call-non-callable]
