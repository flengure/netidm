...
if TYPE_CHECKING:
    from netidm_openapi_client import ApiClient

NETIDM_IDM_ADMIN = "idm_admin"


@pytest.fixture(scope="function")
async def client() -> Optional[NetidmClient]:
    """sets up a client with a basic thing"""
    try:
        basicConfig(level=DEBUG)

        return NetidmClient(uri="https://idm.example.com")
    except FileNotFoundError:
        pytest.skip("Couldn't find config file...")  # type: ignore[call-non-callable]


@pytest.fixture(scope="function")
async def client_configfile() -> Optional[NetidmClient]:
    """sets up a client from a config file"""
    try:
        return NetidmClient(config_file=Path("~/.config/netidm").expanduser())
    except FileNotFoundError:
        pytest.skip("Couldn't find config file...")  # type: ignore[call-non-callable]
...
def openapi_server_url() -> str:
    return os.getenv("NETIDM_OPENAPI_URL", "https://localhost:8443")


@pytest.fixture(scope="session")
def openapi_ca_path() -> Optional[str]:
    env_path = os.getenv("NETIDM_CA_PATH")
    if env_path:
        return env_path
    default_path = "/tmp/netidm/ca.pem"
    return default_path if Path(default_path).exists() else None


@pytest.fixture(scope="session")
def openapi_verify_tls(openapi_ca_path: Optional[str]) -> bool:
    insecure = os.getenv("NETIDM_INSECURE")
...
def openapi_admin_credentials() -> Tuple[str, str]:
    ...
    return (NETIDM_IDM_ADMIN, password)
...
@pytest.fixture(scope="function")
async def openapi_api_client(openapi_client: NetidmClient) -> AsyncIterator["ApiClient"]:
...
@pytest.fixture(scope="function")
async def openapi_api_client_authed(openapi_authed_client: NetidmClient) -> AsyncIterator["ApiClient"]:
