...
def test_parse_config_validationerror(client: NetidmClient) -> None:
...
async def test_parse_config_data(client: NetidmClient) -> None:
...
def test_config_invalid_uri() -> None:
    ...
    with pytest.raises(pydantic.ValidationError):
        NetidmClientConfig.model_validate(test_input)
...
def test_config_loader_str() -> None:
    ...
    with pytest.raises(FileNotFoundError):
        NetidmClient(config_file="hello world")


def test_config_init() -> None:
    """tests passing config object"""

    config = NetidmClientConfig(uri="https://idp.crabzrool.test")
    assert NetidmClient(config=config)
